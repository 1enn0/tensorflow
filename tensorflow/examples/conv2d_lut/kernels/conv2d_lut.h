...
#ifndef TENSORFLOW_EXAMPLES_CONV2DLUT_KERNELS_CONV_2D_H_
#define TENSORFLOW_EXAMPLES_CONV2DLUT_KERNELS_CONV_2D_H_
...
template <typename Device, typename Input, typename Filter, typename LookupTable, 
          typename Output, typename OutputKernel>
void SpatialConvolutionLUTFunc(const Device& d, Output output, 
                            Input input, Filter filter, LookupTable lut, 
  ...
  output.device(d) = Eigen::SpatialConvolutionLUT(
      input, filter, lut, col_stride, row_stride, padding, col_dilation,
...
struct SpatialConvolutionLUT {
  void operator()(const Device& d, typename TTypes<U, 4>::Tensor output,
    ...
    SpatialConvolutionLUTFunc(d, output, input, filter, lut, row_stride, col_stride,
  ...
  void operator()(const Device& d, typename TTypes<U, 4>::Tensor output,
                  ...
                  int padding_right,
                  const OutputKernel& output_kernel = OutputKernel()) {
    SpatialConvolutionLUTFunc(
        d, output, input, filter, lut, row_stride, col_stride, row_dilation,
...
#endif  //  TENSORFLOW_EXAMPLES_CONV2DLUT_KERNELS_CONV_2D_H_
