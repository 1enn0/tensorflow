...
class MatMulLutTest : public OpsTestBase {
 protected:
};

// Runs two small matrices through the operator, and leaves all the parameters
// at their default values.
TEST_F(MatMulLutTest, Small_NoParams) {
  TF_ASSERT_OK(NodeDefBuilder("mat_mul_lut_op", "MatMulLUT")
...
TEST_F(MatMulLutTest, VerySmall_WithParams) {
...
TEST_F(MatMulLutTest, VerySmall_BadRange) {
  // These parameters reflect a typical production usage of eight-bit matmuls
  // in an Inception-style network.
  const bool transpose_a = true;
  const int a_rows = 1;
...
TEST_F(MatMulLutTest, Small_WithParams) {
  // These parameters reflect a typical production usage of eight-bit matmuls
  // in an Inception-style network.
  const bool transpose_a = true;
  const int a_rows = 3;
...
// inputs.
TEST_F(MatMulLutTest, Medium_WithParams) {
  const bool transpose_a = true;
  const bool transpose_b = false;
  TF_ASSERT_OK(NodeDefBuilder("quantized_mat_mul_op", "QuantizedMatMul")
                   .Input(FakeInput(DT_QUINT8))
